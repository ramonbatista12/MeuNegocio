...
class ViewModelCadastroDeCliente@Inject constructor(private val repositorio: Repositorio) : ViewModel() {
    ...
    fun adicionarEndereco(endereco: Endereco){
        coroutineScope.launch {
            try {
                repositorio.inserirEndereco(AuxiliarValidacaoDadosDeClientes().validarEndereco(endereco))
            }catch (e: Exception){
                snackbarHostState.showSnackbar(e.message.toString())
            }
    ...
    fun adicionarTelefone(telefone: Telefone){
        coroutineScope.launch {
            try {
                repositorio.inserirTelefone(AuxiliarValidacaoDadosDeClientes().validarTelefone(telefone))
            }catch (e: Exception){
                snackbarHostState.showSnackbar(e.message.toString())
            }
...
class AuxiliarValidacaoDadosDeClientes{
    ...
    suspend fun validarEndereco(endereco: Endereco?): Endereco{
        ...
        return Endereco(0, idCli = endereco.idCli, cep = cep, bairro = bairo, cidade = cidade, estado = estado, rua = rua, numero = numero, complemento = endereco.complemento)
    }
    ...
    suspend fun validarTelefone(telefone: Telefone?): Telefone{
        ...
        return Telefone(0, idCli = telefone.idCli, ddd = ddd, numero = celular)
