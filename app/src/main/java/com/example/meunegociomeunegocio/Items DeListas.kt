...
@Composable
fun itemProduto(windowSize: WindowSizeClass, modifier: Modifier=Modifier){
     val pd = ProdutoServico(0,true,"agua","gelada fabrica nos alpes suicos na inglatera francessa",1.50f)
     val expandidido = remember{mutableStateOf(false)}

     Card(modifier= Modifier.fillMaxWidth().height(if(expandidido.value) 150.dp else 70.dp).padding(top = 5.dp, start = 3.dp, end = 3.dp),
        ...
        Box(modifier = Modifier.fillMaxSize()){
            ...
            Row(Modifier.padding(top = 3.dp, start = 5.dp, end = 5.dp).align(if(!expandidido.value)Alignment.CenterStart else Alignment.TopStart)) {
                Text(pd.nome, maxLines = 2,modifier=Modifier.width(70.dp))
                Spacer(Modifier.padding(10.dp))
                //if(windowSize.windowWidthSizeClass)
                Text(pd.preco.toString(), Modifier.width(70.dp))
                Spacer(Modifier.padding(10.dp))
                Text(pd.descrisao, maxLines = 1, modifier = Modifier.width(90.dp), overflow = TextOverflow.Ellipsis)
            ...
            Box(Modifier.fillMaxWidth().padding(start = 5.dp)){
                Text(text = pd.descrisao, maxLines = 4,modifier =  Modifier.align(Alignment.CenterStart))
            }}}
...
@Composable
fun ListaDePEdidos(modifier: Modifier=Modifier){
...
fun previaLista(){
    MaterialTheme{
        Surface(Modifier.fillMaxSize().padding(top=50.dp)) {
            Scaffold(){
              ListaDePEdidos(Modifier.padding(it))
